...
栞　get_item　_inventoryNo　_itempass　_itemname
{
   ...
   違ったら　臨時全域変数　inventoryQTE[{{_inventoryNo}}]　= 1
    ...
    //コマンド絵は再定義上書き＝呼び出した領域に依存する
    //コマンド画像はobjはそのまま画像中身を差し替えるだけ＝基底領域固定
...
```
見比べてみるとボタンを呼び出すタイミングが変わり、透明度操作になりましたね。　　
画像はアイテムを持っていないとき用の透明専用画像を用意。コマンド絵ではまく画像コマンドを使用することで基底領域束縛を実現しました
文字は文字窓で宣言することでこれも画面領域耐性を得ています。
